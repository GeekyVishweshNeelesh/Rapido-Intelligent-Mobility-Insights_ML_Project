"""
RAPIDO ML PROJECT - STREAMLIT DASHBOARD
Author: VishweshN
"""
...
# MODEL1_DEFAULTS → retrained model (no actual_ride_time_min, no incomplete_ride_reason)
#                   Uses GOOD customer profile (low cancellation, high completion)
# OTHER_DEFAULTS  → models 2, 3, 4 (unchanged from training medians)
...
MODEL1_DEFAULTS = {
    ...
    'is_weekend'                     : 0,
    'hour_of_day'                    : 12,
    # Ride features (no actual_ride_time_min — removed as leakage)
    'estimated_ride_time_min'        : 55,   # datetime-encoded median
    'ride_distance_km'               : 13.03,
    'base_fare'                      : 187.35,
    'surge_multiplier'               : 1.6,
    'booking_value'                  : 290.09,
    # Customer features — GOOD profile (low cancellation = Completed)
    'cust_customer_age'              : 41,
    'cust_customer_signup_days_ago'  : 520,
    'cust_total_bookings'            : 50,     # high bookings = loyal customer
    'cust_completed_rides'           : 45,     # high completion
    'cust_cancelled_rides'           : 2,      # very few cancellations
    'cust_incomplete_rides'          : 1,
    'cust_cancellation_rate'         : 4.0,    # LOW rate — good customer
    'cust_avg_customer_rating'       : 4.5,
    # Driver features — GOOD profile (low delay = Completed)
    'driver_driver_age'              : 38,
    'driver_driver_experience_years' : 7,
    'driver_total_assigned_rides'    : 21,
    'driver_accepted_rides'          : 16,
    'driver_delay_count'             : 0,      # no delays
    'driver_delay_rate'              : 0.02,   # very low delay rate
    'driver_acceptance_rate'         : 0.90,   # high acceptance
    'driver_cancellation_rate'       : 0.02,
    'driver_total_cancellations'     : 1,
    'driver_incomplete_rides'        : 0,
    'total_risk_factors'             : 0,      # no risk factors
    'pairing_quality_score'          : 75.0,   # high quality pairing
    # Categoricals
    'city'                           : 'Delhi',
    'pickup_location'                : 'Loc_17',
    'drop_location'                  : 'Loc_5',
    'vehicle_type'                   : 'Cab',
    'traffic_level'                  : 'Low',
    'weather_condition'              : 'Clear',
    'cust_customer_gender'           : 'Male',
    'cust_customer_city'             : 'Mumbai',
    'cust_preferred_vehicle_type'    : 'Bike',
    'cust_frequency_category'        : 'Regular',
    'cust_value_segment'             : 'High Value',
    'cust_favorite_city'             : 'Delhi',
    'driver_driver_city'             : 'Mumbai',
    'driver_vehicle_type'            : 'Auto',
    'driver_experience_level'        : 'Beginner',
    'driver_performance_tier'        : 'Gold',
    'driver_primary_city'            : 'Mumbai',
    'rush_hour_type'                 : 'Morning',
    'time_of_day'                    : 'Afternoon',
    'hour_category'                  : 'Morning',
    'fare_category'                  : 'Medium',
    'surge_category'                 : 'No Surge',
    'distance_category'              : 'Medium',
    'city_cleaned'                   : 'Delhi',
    'route'                          : 'Loc_17_to_Loc_5',
    'cust_driver_exp_match'          : 'Mismatched',
}
...
OTHER_DEFAULTS = {
    ...
    'hour_of_day'                    : 12,
    'ride_distance_km'               : 13.03,
    'estimated_ride_time_min'        : 55,
    'actual_ride_time_min'           : 58,
    'base_fare'                      : 187.35,
    'surge_multiplier'               : 1.6,
    'booking_value'                  : 290.09,
    'cust_customer_age'              : 41,
    'cust_customer_signup_days_ago'  : 520,
    'cust_total_bookings'            : 11,
    'cust_completed_rides'           : 7,
    'cust_cancelled_rides'           : 2,
    'cust_incomplete_rides'          : 1,
    'cust_cancellation_rate'         : 22.22,
    'cust_avg_customer_rating'       : 4.3,
    'cust_customer_cancel_flag'      : 1,
    'driver_driver_age'              : 38,
    'driver_driver_experience_years' : 7,
    'driver_total_assigned_rides'    : 21,
    'driver_accepted_rides'          : 16,
    'driver_delay_count'             : 1,
    'driver_delay_rate'              : 0.05,
    'driver_acceptance_rate'         : 0.76,
    'driver_cancellation_rate'       : 0.05,
    'driver_total_cancellations'     : 2,
    'driver_incomplete_rides'        : 1,
    'total_risk_factors'             : 1,
    'pairing_quality_score'          : 0.5,
    ...
    'cust_customer_city'             : 'Mumbai',
    'cust_preferred_vehicle_type'    : 'Bike',
    'cust_frequency_category'        : 'Occasional',
    'cust_value_segment'             : 'Medium Value',
    'cust_favorite_city'             : 'Delhi',
    'driver_driver_city'             : 'Mumbai',
    'driver_vehicle_type'            : 'Auto',
    'driver_experience_level'        : 'Beginner',
    'driver_performance_tier'        : 'Silver',
    'driver_primary_city'            : 'Mumbai',
    'rush_hour_type'                 : 'Morning',
    'time_of_day'                    : 'Afternoon',
    'hour_category'                  : 'Morning',
    'fare_category'                  : 'Medium',
    'surge_category'                 : 'Low Surge',
    'distance_category'              : 'Medium',
    'city_cleaned'                   : 'Delhi',
    'route'                          : 'Loc_17_to_Loc_5',
    'cust_driver_exp_match'          : 'Mismatched',
...
# ── datetime-encoded time columns (Models 2,3,4 still have these) ──
def encode_time_minutes(minutes, encoder):
    ...
    try:
        ...
        target_clamped = f'1970-01-01 00:00:00.000000{clamped:03d}'
        if target_clamped in classes:
            return classes.index(target_clamped)
    except Exception:
        pass
    return len(classes) // 2  # fallback to middle index
...
def prepare_input(user_inputs, model_num, models):
    try:
        ...
        # Step 1: Fill with model-specific defaults
        ...
        if encoders:
            for col, encoder in encoders.items():
                ...
                # datetime-encoded time columns (models 2,3,4 only now)
...
def render_sidebar():
    with st.sidebar:
        ...
        if os.path.exists(CSV_PATH):
            st.success(f"✅ CSV Ready ({os.path.getsize(CSV_PATH)/1024/1024:.1f} MB)")
        else:
        ...
        st.markdown("---")
        st.markdown("<div style='text-align:center;color:#555;font-size:11px;'>Rapido ML v1.0</div>", unsafe_allow_html=True)
    return page

def get_common_inputs(suffix="", model_num=1):
    # Use model-specific defaults for UI widgets
    defaults = get_defaults(model_num)
    surge_default = defaults.get('surge_multiplier', 1.6)
    fare_default  = defaults.get('base_fare', 187.0)
    traffic_default   = defaults.get('traffic_level', 'High')
    weather_default   = defaults.get('weather_condition', 'Heavy Rain')
    traffic_options   = ["Low","Medium","High"]
    weather_options   = ["Clear","Rain","Heavy Rain"]
    traffic_idx = traffic_options.index(traffic_default) if traffic_default in traffic_options else 2
    weather_idx = weather_options.index(weather_default) if weather_default in weather_options else 2
    ...
    with c3:
        ...
        traffic_level     = st.selectbox("Traffic Level",    traffic_options, index=traffic_idx, key=f"traf{suffix}")
        weather_condition = st.selectbox("Weather Condition",weather_options, index=weather_idx, key=f"weath{suffix}")
    ...
    return {
        ...
        'actual_ride_time_min'   : estimated_ride_time_min,  # same as estimated for Model 1
        ...
        'base_fare'              : base_fare,
        'booking_value'          : round(base_fare * surge_multiplier, 2),
    }

# ── PAGE 1: HOME ──
def page_home():
    st.markdown("<h1>🛵 Rapido: Intelligent Mobility Insights</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color:#888;font-size:16px;'>Ride Patterns, Cancellations & Fare Forecasting</p>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    for col,(t,s,v,m,st_,c_) in zip([c1,c2,c3,c4], cards):
        with col:
            st.markdown(f"""<div class='info-card'>
                ...
                <p style='color:#888;margin:4px 0;'>{s}</p>
    ...
    with c1:
        ...
        <p>Model 3 identifies high-risk bookings before they get cancelled.</p></div>
        <div class='info-card'><h3 style='color:#ff6b00;'>⏱️ Improve ETA Accuracy</h3>
        <p>Model 4 predicts delay-prone drivers to improve arrival estimates.</p></div>
        """, unsafe_allow_html=True)
    with c2:
        st.markdown("""
        <div class='info-card'><h3 style='color:#ff6b00;'>💰 Dynamic Pricing</h3>
        <p>Model 2 estimates accurate fares based on distance, traffic, and surge.</p></div>
        <div class='info-card'><h3 style='color:#ff6b00;'>🚗 Driver Reliability Scoring</h3>
        <p>Model 4 scores driver reliability to improve allocation strategy.</p></div>
        """, unsafe_allow_html=True)
    st.markdown("---")
    st.markdown("<h2>🛠️ Tech Stack</h2>", unsafe_allow_html=True)
    c1,c2,c3,c4,c5,c6 = st.columns(6)
    for col,(icon,name) in zip([c1,c2,c3,c4,c5,c6],
        [("🐍","Python"),("🤖","Scikit-Learn"),("⚡","XGBoost"),
         ("💡","LightGBM"),("🗄️","MariaDB"),("🌐","Streamlit")]):
        with col:
            st.markdown(f"<div class='info-card' style='text-align:center;'><div style='font-size:24px;'>{icon}</div><b>{name}</b></div>", unsafe_allow_html=True)
...
def page_eda():
    ...
    with tab1:
        ...
        with c1:
            if 'booking_status' in df.columns:
                ...
                fig = px.pie(cnt, values='Count', names='Status', title='Booking Status Distribution',
                             color_discrete_sequence=['#ff6b00','#00cc44','#ff4444','#4488ff'])
                fig.update_layout(**T)
        ...
        with c2:
            if 'vehicle_type' in df.columns:
                ...
                fig.update_layout(**T)
                st.plotly_chart(fig, use_container_width=True)
        hour_col = next((c for c in ['hour_of_day','booking_hour','hour'] if c in df.columns), None)
        if hour_col:
            ...
            fig.update_layout(**T, xaxis_title='Hour', yaxis_title='Rides')
            st.plotly_chart(fig, use_container_width=True)
        dow_col = next((c for c in ['day_of_week','booking_day_of_week','weekday'] if c in df.columns), None)
        if dow_col:
            day_order = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']
            df2 = df.copy()
            df2['_day'] = df2[dow_col].apply(safe_day_label)
            d = df2.groupby('_day').size().reset_index(name='Rides')
            d['_ord'] = d['_day'].apply(lambda x: day_order.index(x) if x in day_order else 99)
            ...
            fig.update_layout(**T, xaxis_title='Day', yaxis_title='Rides')
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        ...
        with c1:
            if fare_col:
                fd = df[fare_col].dropna()
                fd = fd[(fd > 0) & (fd < fd.quantile(0.99))]
    ...
    with tab4:
        ...
        if 'weather_condition' in df.columns:
            wc = df['weather_condition'].value_counts().reset_index()
            wc.columns = ['Weather','Count']
            fig = px.bar(wc, x='Weather', y='Count', title='Weather Condition Distribution',
...
def page_ride_outcome():
    ...
    with ex1:
        inputs['cust_total_bookings']      = st.number_input("Total Past Bookings", 0,500, 50, 1, key="ctb_m1")
    with ex2:
        inputs['cust_completed_rides']     = st.number_input("Completed Rides",     0,500, 45, 1, key="ccr_m1")
    with ex3:
        inputs['cust_cancelled_rides']     = st.number_input("Cancelled Rides",     0,500,  2, 1, key="ccan_m1")
    with ex4:
        inputs['cust_avg_customer_rating'] = st.number_input("Avg Rating",        1.0,5.0,4.5,0.1, key="crat_m1")

    st.markdown("**🚗 Driver Quality**")
    dr1,dr2,dr3 = st.columns(3)
    with dr1:
        inputs['driver_delay_rate']      = st.number_input("Driver Delay Rate",      0.0,1.0,0.02,0.01, key="ddr_m1")
    with dr2:
        inputs['driver_acceptance_rate'] = st.number_input("Driver Acceptance Rate", 0.0,1.0,0.90,0.01, key="dar_m1")
    with dr3:
        inputs['driver_delay_count']     = st.number_input("Driver Delay Count",     0,50, 0,   1,      key="ddc_m1")

    st.markdown("---")

    if st.button("🎯 PREDICT RIDE OUTCOME"):
        # Derive cancellation rate from inputs
        if inputs['cust_total_bookings'] > 0:
            inputs['cust_cancellation_rate'] = (
                inputs['cust_cancelled_rides'] / inputs['cust_total_bookings']) * 100
        ...
        if scaled is not None:
            ...
            with c1:
                st.markdown(f"<div class='prediction-box {box}'>{outcome}</div>",
                            unsafe_allow_html=True)
                conf = float(proba[0][predicted_class]) * 100
                st.markdown(f"<div style='text-align:center;color:#888;margin-top:10px;'>Confidence: <b style='color:#ff6b00;'>{conf:.1f}%</b></div>",
                            unsafe_allow_html=True)
            with c2:
                labels = [class_names.get(int(c), str(c)) for c in classes]
                colors = ['#00cc44' if 'Completed' in l else '#ff4444' for l in labels]
...
def page_cancellation_risk():
    ...
    with ec1:
        inputs['cust_customer_age']             = st.number_input("Customer Age",       18,80,  41, 1,   key="cage_m3")
    with ec2:
        inputs['cust_customer_signup_days_ago'] = st.number_input("Account Age (days)", 0,3000,520, 1,   key="csignup_m3")
    with ec3:
        inputs['cust_total_bookings']           = st.number_input("Total Bookings",     0,500,  11, 1,   key="ctotal_m3")
    with ec4:
        inputs['cust_completed_rides']          = st.number_input("Completed Rides",    0,500,   7, 1,   key="ccr_m3")
    ec5,ec6 = st.columns(2)
    with ec5:
        inputs['cust_avg_customer_rating'] = st.number_input("Avg Rating", 1.0,5.0,4.3,0.1, key="crat_m3")
    with ec6:
        inputs['cust_customer_gender'] = st.selectbox("Customer Gender",
            ["Male","Female","Non-Binary"], key="cgender_m3")
    st.markdown("---")
    if st.button("🚫 PREDICT CANCELLATION RISK"):
        if inputs['cust_total_bookings'] > 0:
            inputs['cust_cancellation_rate'] = (
                (inputs['cust_total_bookings'] - inputs['cust_completed_rides'])
                / inputs['cust_total_bookings']) * 100
        ...
        if scaled is not None:
            ...
            with c1:
                ...
                fn(adv)
            with c2:
                st.plotly_chart(gauge_chart(cancel_prob, "Cancellation Probability (%)"),
...
def page_driver_delay():
    ...
    with dp1:
        inputs['driver_performance_tier']    = st.selectbox("Performance Tier",
            ["Bronze","Silver","Gold","Platinum"], index=1, key="dtier_m4")
    with dp2:
        inputs['driver_experience_level']    = st.selectbox("Experience Level",
            ["Newbie","Beginner"], index=1, key="dexp_m4")
    with dp3:
        inputs['driver_total_assigned_rides']= st.number_input("Total Assigned Rides",0,1000,21,1, key="dtar_m4")
    with dp4:
        inputs['driver_delay_count']         = st.number_input("Past Delay Count",    0,100,  1,1, key="ddc_m4")
    st.markdown("---")
    if st.button("⏱️ PREDICT DRIVER DELAY"):
        ...
        if scaled is not None:
            ...
            with c1:
                ...
                fn(adv)
            with c2:
                st.plotly_chart(gauge_chart(delay_prob, "Delay Probability (%)"),
                                use_container_width=True)
...
def page_model_performance():
    ...
    fallback = {
        1: pd.DataFrame({'Algorithm':['Logistic Regression','Random Forest','XGBoost','LightGBM'],
                         'Accuracy' :[0.72,  0.85,  0.87, 0.86],
                         'F1-Score' :[0.71,  0.84,  0.86, 0.85]}),
        2: pd.DataFrame({'Algorithm':['Linear Regression','Random Forest','XGBoost'],
                         'RMSE (%)' :[31.76, 19.98, 9.92],
                         'R² Score' :[0.69,  0.88,  0.9696]}),
        3: pd.DataFrame({'Algorithm':['Logistic Regression','Random Forest','XGBoost','LightGBM'],
                         'Accuracy' :[0.7324,0.9165,0.9252,0.9271],
                         'AUC'      :[0.8138,0.9648,0.9837,0.9841]}),
        4: pd.DataFrame({'Algorithm':['Logistic Regression','Random Forest','XGBoost','LightGBM'],
                         'Accuracy' :[0.5818,0.6862,0.7938,0.8304],
                         'AUC'      :[0.6188,0.7564,0.8908,0.9239]}),
    }
    summaries = {
    ...
    for tab,num in zip([tab1,tab2,tab3,tab4],[1,2,3,4]):
        with tab:
            ...
            with c2:
                ...
                fig = px.bar(df_comp, x='Algorithm', y=y_col,
                             color=y_col, color_continuous_scale='Oranges',
                             title=f'{y_col} Comparison')
